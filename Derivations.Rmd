...
---
\newcommand{\Yit}{Y_{it}}
\newcommand{\Yit1}{Y_{it-1}}
\newcommand{\Yitk}{Y_{it,k}}
\newcommand{\YitK}{Y_{it,K}}
\newcommand{\Yitl}{Y_{it,l}}
\newcommand{\Yitd}{Y_{it,d}}
\newcommand{\Yitb}{Y_{it,b}}
\newcommand{\Yit1l}{Y_{it-1,l}}
\newcommand{\Yi1k}{Y_{i1,k}}
\newcommand{\yitl}{y_{it,l}}
\newcommand{\yit1l}{y_{it-1,l}}
\newcommand{\Zitk}{Z_{it,k}}
\newcommand{\Zi1k}{Z_{i1,k}}
\newcommand{\Zitkp1}{Z_{it,k+1}}
\newcommand{\Zi1kp1}{Z_{i1,k+1}}
...
\newcommand{\mumitK1}{\mu_{it,K-1}^m}
\newcommand{\mumitk}{\mu_{it,k}^m}
\newcommand{\mumi1k}{\mu_{i1,k}^m}
...
\newcommand{\pimitk1}{\pi_{it,k-1}^m}

\newcommand{\pimitkp1}{\pi_{it,k+1}^m}
\newcommand{\pimi1k}{\pi_{i1,k}^m}
\newcommand{\pimi1kp1}{\pi_{i1,k+1}^m}
...
\newcommand{\etai1k}{\eta_{i1,k}}
\newcommand{\etai1kp1}{\eta_{i1,k+1}}
...
\newcommand{\DeltaitK1}{\Delta_{it,K-1}}
...
\newcommand{\gammaK1g}{\gamma^{(K-1)g}}
\newcommand{\gammaK1K1}{\gamma^{(K-1)(K-1)}}
...
\newcommand{\gammaK1ga}{\gamma^{(K-1)g(a)}}
\newcommand{\gammaK1K1a}{\gamma^{(K-1)(K-1)(a)}}
\newcommand{\gamma11a}{\gamma^{11(a)}}
\newcommand{\gamma1g1}{\gamma^{1g(1)}}
\newcommand{\gammaK1g1}{\gamma^{(K-1)g(1)}}
\newcommand{\gamma1gA}{\gamma^{1g(A)}}
\newcommand{\gammaK1gA}{\gamma^{(K-1)g(A)}}


\newcommand{\phiitk}{\phi_{it,k}}
\newcommand{\phiitkp1}{\phi_{it,k+1}}
\newcommand{\phii1k}{\phi_{i1,k}}
\newcommand{\phii1kp1}{\phi_{i1,k+1}}
...
$\mumitk = pr(\Yit=k) = \pimitk - \pimitk1$ is the marginal probability of being in state $k$.

$\mucitk = pr(\Yit=k \mid \Yit1)$ is the conditional (on the lagged response) probability of being in state $k$.  It captures how $Y$s depend on each other.  Here we are assuming a first-order Markov model.

$\hitkl = pr(\Yit=k \mid \Yit1=l)$ where conditioning on $\uit$ is implicit
...
log \left( \frac{\mucitk}{\mucitK} \right) &= \Deltaitk(\mathbf{X}_i) + \sum_{l=1}^{K-1} \uit \gammakl \yit1l
\end{aligned}
$$
...
Since the marginal mean model must equal the marginalized, conditional mean model (where we marginalize over the distribution of $\Yit1$), then
...
* $\hitkg = pr(\Yitk \mid \Yit1=g) = \frac{exp(\Deltaitk + \uit \gammakg)}{1+\sum_{l=1}^{K-1} exp(\Deltaitl + \uit \gammalg)}$
* $\hitkK = pr(\Yitk \mid \Yit1=K)= \frac{exp(\Deltaitk)}{1+\sum_{l=1}^{K-1} exp(\Deltaitl)}$ because state $K$ is the reference state for lagged values (i.e. $\gammakK=0$). So $\gamma$ does not appear in the $K^{th}$ column of the $\mathbf{h}_{it}$ matrix.
* $\hitKg =  1-\sum_{k=1}^{K-1} pr(\Yitk \mid \Yit1=g) = 1-\sum_{k=1}^{K-1} \hitkg = \frac{1}{1+\sum_{l=1}^{K-1} exp(\Deltaitl+ \uit \gammalg)}$ because state $K$ is the reference state for the outcome
* In the $\mathbf{h}_{it}$ matrix ($K \times K$), rows correspond to $\Yit$ and columns correspond to $\Yit1$, so that the $(k,g)^{th}$ element is $\hitkg = pr(\Yitk \mid \Yit1=g)$.
...
* $log \left( \frac{ pr(\Yit=k \mid \Yit1=g)}{pr(\Yit=K \mid \Yit1=g)} \right) = \Deltaitk + \uit \gammakg$ and $log \left( \frac{ pr(\Yit=k \mid \Yit1=K)}{pr(\Yit=K \mid \Yit1=K)} \right) = \Deltaitk$, so


$$\gammakg=  log \left( \frac{ pr(\Yit=k \mid \Yit1=g)/pr(\Yit=K \mid \Yit1=g)  }{ pr(\Yit=k \mid \Yit1=K)/pr(\Yit=K \mid \Yit1=K) } \right)  = log \left( \frac{ \hit{k}{g} / \hit{K}{g}  }{  \hit{k}{K} / \hit{K}{K} } \right)$$
...
=& \sum_{k=1}^{K} \Yi1k log (\mumi1k) + \sum_{t=2}^{T_i} \sum_{k=1}^{K} \Yitk log (\mucitk)
\end{aligned}
$$
...
$$
\begin{aligned}
l_i^{(1)}(\mathbf{\beta},\mathbf{\alpha})= \sum_{k=1}^{K-1} [\Zi1k \phii1k - \Zi1kp1 g(\phii1k)] 
\end{aligned}
$$

where

* $\phiitk = log \left( \frac{\pimitk}{\pimitkp1-\pimitk} \right)$
* $g(\phiitk) = log(1+exp(\phiitk)) = log \left( \frac{\pimitkp1}{\pimitkp1-\pimitk} \right)$
...
$$
\begin{aligned}
l_i^{(2)}(\mathbf{\theta})= \sum_{t=2}^{T_i} \left[ \sum_{k=1}^{K-1} \Yitk (\Deltaitk + \sum_{l=1}^{K-1} \uit \gammakl \Yit1l) - log(1+\sum_{k=1}^{K-1} exp(\Deltaitk + \sum_{l=1}^{K-1} \uit \gammakl \Yit1l)) \right] 
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial l_i^{(1)}(\mathbf{\beta},\mathbf{\alpha})}{\partial \theta_1}= \sum_{k=1}^{K-1} \left[ \Zi1k \frac{\partial \phii1k}{\partial \theta_1} - \Zi1kp1 \frac{\partial g(\phii1k)}{\partial \theta_1} \right]  
\end{aligned}
$$

where we can show that (see handwritten notes)

* $\frac{\partial \phii1k}{\partial \theta_1}=exp(g(\phii1k)) \cdot \left[(1-\pimi1k) \frac{\partial \etai1k}{\partial \theta_1} - (1-\pimi1kp1) \frac{\partial \etai1kp1}{\partial \theta_1}   \right]$
* $\frac{\partial g(\phii1k)}{\partial \theta_1}=exp(\phii1k) \cdot \left[(1-\pimi1k) \frac{\partial \etai1k}{\partial \theta_1} - (1-\pimi1kp1) \frac{\partial \etai1kp1}{\partial \theta_1}   \right] = \frac{exp(\phii1k)}{exp(g(\phii1k))} \cdot \frac{\partial \phii1k}{\partial \theta_1} = \frac{\pimi1k}{\pimi1kp1} \cdot \frac{\partial \phii1k}{\partial \theta_1}$

so that 

$$
\begin{aligned}
\frac{\partial l_i^{(1)}(\mathbf{\beta},\mathbf{\alpha})}{\partial \theta_1}= \sum_{k=1}^{K-1} \left[ \Zi1k  - \Zi1kp1 \frac{\pimi1k}{\pimi1kp1} \right]  \frac{\partial \phii1k}{\partial \theta_1}
\end{aligned}
$$

where 

* $\frac{\pimi1k}{\pimi1kp1}=\frac{exp(\phii1k)}{1+exp(\phii1k)}$
* $\frac{\partial \phii1k}{\partial \theta_1} = \frac{\partial \phii1k}{\partial \pimi1k} \cdot \frac{\partial \pimi1k}{\partial \etai1k} \cdot \frac{\partial \etai1k}{\partial \theta_1} + \frac{\partial \phii1k}{\partial \pimi1kp1} \cdot \frac{\partial \pimi1kp1}{\partial \etai1kp1} \cdot \frac{\partial \etai1kp1}{\partial \theta_1} =  \frac{\partial \phii1k}{\partial \etai1k} \cdot \frac{\partial \etai1k}{\partial \theta_1} + \frac{\partial \phii1k}{\partial \etai1kp1} \cdot \frac{\partial \etai1kp1}{\partial \theta_1}$.  This is from LD2007 and I checked that this is equal to what I got.


<!-- $$ -->
<!-- \begin{aligned} -->
<!-- # \frac{\partial l_i^{(2)}(\mathbf{\theta})}{\partial \theta_1}= & \sum_{t=2}^{T_i} \left[ \sum_{k=1}^{K-1} \Yitk \left(\frac{\partial \Deltaitk}{\partial \theta_1} + \sum_{l=1}^{K-1} \frac{\partial \gammakl}{\partial \theta_1} \Yit1l \right)  - \frac{exp(\Deltaitk + \sum_{l=1}^{K-1} \gammakl \Yit1l)}{(1+\sum_{k=1}^{K-1} exp(\Deltaitk + \sum_{l=1}^{K-1} \gammakl \Yit1l))} \left(\frac{\partial \Deltaitk}{\partial \theta_1} + \sum_{l=1}^{K-1} \frac{\partial \gammakl}{\partial \theta_1} \Yit1l \right) \right] \\  -->
<!-- \end{aligned} -->
<!-- $$ -->
...
$$
\begin{aligned}
\frac{\partial l_i^{(2)}(\mathbf{\theta})}{\partial \theta_1} =& \sum_{t=2}^{T_i} \left[ \sum_{k=1}^{K-1} (\Yitk - \mucitk) \left(\frac{\partial \Deltaitk}{\partial \theta_1} + \sum_{l=1}^{K-1} \uit \frac{\partial \gammakl}{\partial \theta_1} \Yit1l \right)  \right]
\end{aligned}
$$
...
\end{aligned}
$$
Since $\hitkg=h(\Deltait1, \dots \DeltaitK1, \gamma1g1, \dots \gammaK1g1, \dots \gamma1gA, \dots \gammaK1gA)$
